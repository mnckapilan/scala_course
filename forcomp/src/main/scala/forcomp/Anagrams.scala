...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (SortedMap[Char, Int]() ++
      w.toLowerCase.groupBy(identity).map { case (x, xs) => (x, xs.length) }).toList
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.foldRight(List[Occurrences](Nil)) {
      case ((c, t), acc) =>
        acc ++ (for {combi <- acc; n <- 1 to t} yield (c, n) :: combi)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    lazy val yCount = y.foldLeft(Map[Char, Int]())((acc, e) => acc + (e._1 -> e._2))
    x.map(o => (o._1, o._2 - yCount.getOrElse(o._1, 0))).filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def findAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        sentence <- findAnagrams(subtract(occurrences, wordOccurrences(word)))
        if combo.nonEmpty
      } yield word :: sentence
    }
